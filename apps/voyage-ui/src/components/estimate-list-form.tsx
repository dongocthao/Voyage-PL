...
type EstimateType = "Voyage Charter" | "Time Charter" | "Cargo Relet";
type EstimateStatus = "Draft" | "Estimated" | "Fixed" | "Failed" | "Cancelled";
...
const ESTIMATE_TYPES: EstimateType[] = ["Voyage Charter", "Time Charter", "Cargo Relet"];
const STATUSES: EstimateStatus[] = ["Draft", "Estimated", "Fixed", "Failed", "Cancelled"];

const BASE_COLUMNS: ColumnsType<EstimateRow> = [
  {
    title: "Estimate Id",
    dataIndex: "id",
    key: "id",
  ...
  { title: "Estimate Type", dataIndex: "type", key: "type", width: 104 },
  ...
  { title: "Charterer", dataIndex: "charterer", key: "charterer", width: 116 },
  {
    ...
    render: (status: EstimateStatus) => <span>{status}</span>,
  },
...
function toEstimateRow(row: EstimateListRow): EstimateRow {
  return {
    key: row.estimateId,
    id: row.estimateId,
    year: getBusinessYear(row),
    type: row.estimateType,
    vessel: row.vessel,
    voyage: row.voyageNo,
...
function getBusinessYear(row: EstimateListRow) {
  return (
    row.estimateId.match(/\b(20\d{2})\b/)?.[1] ??
...
function uniqueValues(rows: EstimateRow[], field: keyof EstimateRow) {
...
export function EstimateListForm({
  ...
  useEffect(() => {
    ...
    fetchEstimateList()
      .then((items) => {
        if (!alive) return;
        const nextRows = items.map(toEstimateRow);
        setRows(nextRows);
  ...
  return (
    <StyleProvider hashPriority="high">
      <ConfigProvider theme={veTheme}>
        <div
          className="estimate-list-form flex h-full min-h-0 flex-col bg-white text-black"
          style={{ fontFamily: VE_FONT_FAMILY, fontSize: 11 }}
        >
          <style>
            {`
              .estimate-list-form .ant-table-thead > tr > th {
              ...
              .estimate-list-form .ant-table-tbody > tr > td,
              .estimate-list-form .ant-table-summary > tr > td {
                border-inline-end: 1px solid #d8e2ea !important;
                border-bottom: 1px solid #d8e2ea !important;
              }

              .estimate-list-form .ant-table-tbody > tr > td:last-child,
              .estimate-list-form .ant-table-thead > tr > th:last-child,
              .estimate-list-form .ant-table-summary > tr > td:last-child {
                border-inline-end: 0 !important;
              }

              .estimate-list-form .estimate-status-cell {
                text-align: center;
                font-weight: 600;
              }

              .estimate-list-form .ant-table-tbody > tr > td.estimate-status-draft {
                background: #ffe9c7 !important;
                color: #8a4f00;
              }

              .estimate-list-form .ant-table-tbody > tr > td.estimate-status-estimated {
                background: #dceeff !important;
                color: #0e5d80;
              }

              .estimate-list-form .ant-table-tbody > tr > td.estimate-status-fixed {
                background: #ddf4d7 !important;
                color: #236b1d;
              }

              .estimate-list-form .ant-table-tbody > tr > td.estimate-status-failed {
                background: #eadcff !important;
                color: #5d2c9b;
              }

              .estimate-list-form .ant-table-tbody > tr > td.estimate-status-cancelled {
                background: #ffdada !important;
                color: #9a1f1f;
              }

              .estimate-list-form .ant-table-tbody > tr.ve-row-selected > td.estimate-status-draft,
              .estimate-list-form .ant-table-tbody > tr.ve-row-selected > td.estimate-status-estimated,
              .estimate-list-form .ant-table-tbody > tr.ve-row-selected > td.estimate-status-fixed,
              .estimate-list-form .ant-table-tbody > tr.ve-row-selected > td.estimate-status-failed,
              .estimate-list-form .ant-table-tbody > tr.ve-row-selected > td.estimate-status-cancelled {
          ...
          <div className="shrink-0 border-b border-[#d8e2ea] bg-white px-2 pb-2 pt-1">
            ...
            >
              ...
              <label className="truncate text-[11px] font-semibold text-[#334e63]">Estimate Type</label>
              ...
              <Select size="small" value={filters.type} onChange={setFilter("type")} allowClear placeholder="All" options={toOptions(ESTIMATE_TYPES)} />
                  ...
                  <Table.Summary.Row>
                    <Table.Summary.Cell index={0} colSpan={9}>
                      <div className="flex min-w-0 items-center gap-4 text-[11px] text-[#0e4f68]">
                        <span>
                          Estimated ID: <strong>{selectedRow?.id ?? ""}</strong>
                        </span>
                        <span>
...
export default EstimateListForm;
